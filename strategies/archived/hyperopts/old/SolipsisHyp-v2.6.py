...
class Solipsis6Hyp(IHyperOpt):
        ...
        def populate_buy_trend(dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            conditions.append(
                (
                    ...
                    dataframe['bbdelta'].gt(dataframe['close'] * params['base-bbdelta']) &
                    dataframe['closedelta'].gt(dataframe['close'] * params['base-closedelta']) &
                    dataframe['tail'].lt(dataframe['bbdelta'] * params['base-tail']) &
                    dataframe['close'].lt(dataframe['lower'].shift()) &
                    dataframe['close'].le(dataframe['close'].shift())
                ) |
                (
                    (dataframe['close'] < dataframe['ema_slow']) &
                    (dataframe['close'] < (params['base-bblower'] * dataframe['bb_lowerband'])) &
                    (dataframe['volume'] < (dataframe['volume_mean_slow'].shift(1) * params['base-vms']))
                )
            )
    ...
    def dynamic_roi_space() -> List[Dimension]:
        ...
        return [
            ...
            Real(0.0, 1.0, name='droi_min_roc_atr')
        ]
