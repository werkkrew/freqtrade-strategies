...
class HackieHyp(IHyperOpt):
    ...
    def indicator_space() -> List[Dimension]:
        return [
            Real(0.700, 1.110, name='buy-rmax1'),
            Real(0.699, 1.099, name='buy-rmax2'),
    ...
    def buy_strategy_generator(params: Dict[str, Any]) -> Callable:
        ...
        def populate_buy_trend(dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            dataframe.loc[
                (   (params['buy-rmax1']> params['buy-rmax2']) &
                    (dataframe['fisher-rsi'].lt(params['fisher'])) &
                    (dataframe['ha_high'] >= (dataframe['rmax'] * params['buy-rmax1'])) &
                    (dataframe['ha_high'].shift() >= (dataframe['rmax']) * params['buy-rmax2'])
                ) &
                ((      
                        ...
                        (dataframe['ha_close'].lt(dataframe['lower-bb1'].shift())) &
                        (dataframe['ha_close'].le(dataframe['ha_close'].shift())) 
                ) |
                (       
                        (dataframe['ha_close'] < dataframe['ema_slow']) &
                        (dataframe['ha_close'] < params['close-bblower'] * dataframe['lower-bb2']) &
    ...
    def sell_strategy_generator(params: Dict[str, Any]) -> Callable:
        ...
        def populate_sell_trend(dataframe: DataFrame, metadata: dict) -> DataFrame:
            ...
            dataframe.loc[
            (
                    #(dataframe['ha_close'] > dataframe['rmax']) &
